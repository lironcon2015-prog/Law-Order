...
## ⚠️ בדיקת שפיות בתחילת סשן ולפני מסירה — הוראת קבע
`node tools/repo-audit.mjs` (רץ אוטומטית ב-SessionStart hook, `.claude/hooks/session-start.sh`).
הוא נולד משתי טעויות שכבר קרו, ושתיהן חוזרות אם לא בודקים:
1. **ענף עם עבודה שלא מוזגה ל-main** — נמסר למשתמש קובץ אופליין "מעודכן" שנבנה מ-main,
   בזמן ש-16 קומיטים ישבו בענף אחר. **לפני מסירת קובץ: אם הבדיקה מתריעה, לבדוק מה יש שם
   ולדווח למשתמש** — לא למסור קובץ בלי לוודא שאין עבודה חדשה יותר.
2. **DB_VERSION שיורד** — ענף אחר העלה את גרסת ה-IndexedDB, וגרסה נמוכה יותר **חוסמת לגמרי**
   את האפליקציה אצל המשתמש (`VersionError`). מספר גרסת ה-DB הוא מרחב גלובלי חוצה-ענפים:
   לעולם לא להוריד, רק להעלות, ולשמור את ה-stores שנוצרו בענף האחר.
הבדיקה מתריעה גם כשקובץ אופליין ישן מהקוד שממנו הוא נבנה.
...
## ⚠️ יישור מספרים ב-RTL — הוראת קבע (באג חוזר)
האפליקציה RTL, אבל לכל תא מספרי יש `direction: ltr` (כדי שמטבע, סימן ומפריד אלפים
יופיעו נכון). לכן **`text-align: start` או ברירת המחדל מיישרים אותו לשמאל** בעוד
הכותרת בעברית מיושרת לימין — והמספר לא יושב מתחת/מול הכותרת. זה הבאג שחזר שלוש פעמים.

**הכלל:** בכל מקום שמציג מספר — לעולם לא להסתמך על `start`/ברירת מחדל. יישור פיזי מפורש:
- **טבלאות:** `th`/`td` עם `text-align: right` (לא `start`).
- **מחסנית תווית↑/ערך↓** (`.total`, `.fact`, `.strip__item`, `.kpi__body`, `.team__stat`,
  `.dcard__nums > div`): `text-align: right` על **המכל**, לא על הערך.
- כל דפוס חדש כזה — להוסיף לרשימה ב-`css/styles.css` באותה שורה.

**אימות חובה אחרי כל שינוי UI שמציג מספרים:**
`python3 -m http.server 8765` ואז `node budget/tools/align-audit.mjs` — **חייב להחזיר 0**.
הסקריפט מודד את תיבת הגליפים (`Range.getBoundingClientRect`, ולא תיבת האלמנט —
היא נמתחת על כל התא ולכן מסתירה את הבאג) ומשווה קצה ימני של כותרת מול הערכים,
בכל המסכים, גם במחסניות וגם בעמודות טבלה.
...
- **חותמת גרסה (LexBudget):** `node budget/build-offline.mjs` כותב אוטומטית את `budget/js/version.js`
  (תאריך בנייה, commit, גרסת SW, ערוץ web/offline) — הוא מוצג במסך ההגדרות תחת "גרסה".
  לכלול את `version.js` המעודכן ב-commit, כדי שגם האתר החי יציג חותמת נכונה.
- **⚠️ מסירה בסוף כל סשן — חובה, בלי לחכות לבקשה:** בכל סשן שבו שונה קוד ב-`v2/` או ב-`budget/`,
  לבנות מחדש את קובץ האופליין הרלוונטי ו**לשלוח אותו למשתמש כקובץ מצורף** (`SendUserFile`)
  לפני סיום התשובה. שונו שניהם — לשלוח את שניהם. הקובץ עצמאי לחלוטין (פונטים/CSS/JS מוטמעים),
  ולכן זו הדרך שבה המשתמש מקבל את הגרסה המעודכנת בלי שרת ובלי רשת.
