...
- [x] דשבורד מאוחד בתצוגת "דשבורד" (mainView `today`): KPI **hero** (יתרה לתשלום), KPIs (הכנסה/פתיחה/תשלומים/עמלות), BarChart תזרים חודשי (עמלות vs תשלומים), Donut עמלות לפי סוג תיק, וקטע **"היום"** של ה-CRM (`ui.buildToday`). בלי טבלאות.
- [x] דף **אנליזה** נפרד (sidebar): טבלת סיכום לפי לקוח + סיכום לפי סוג תיק.
- [x] אימות Chromium + seed: דשבורד + אנליזה, נתונים עקביים (עמלות 12,800 / הכנסה 160,000), 0 שגיאות. SW `unified-v5`.
