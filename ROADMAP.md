...
- [x] **PDF — חילוץ שדות מלא לפי תוויות** (`extractInvoiceFields`): מספר חשבונית (`חשבון עסקה N`), תאריך (`תאריך:`), שם לקוח (`שם לקוח:`/`לכבוד:`), מזהה לקוח חיצוני, סוג תיק, וסכומים תווית-מונחים — **לפני מע״מ** (בסיס העמלה, ברירת מחדל), מע״מ, וסה״כ-כולל-מע״מ (בהערה). נרמול גרשיים מסולסלים של pdf.js. **מסמך רב-עמודי מקובץ לפי מספר חשבונית** (`classifyPdfDocument`) → חשבון+נספח = מועמד אחד (לא כפילות).
...
- [x] **יצירת לקוח אוטומטית**: לקוח שזוהה במסמך ואינו במערכת → אפשרות "➕ צור לקוח" **שנבחרת אוטומטית** במסך הסיווג; בשמירה מוקם לקוח+תיק (עם סוג התיק שזוהה) והחשבונית משויכת.
