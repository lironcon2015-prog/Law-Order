...
- הפרדת "שדה הזנה מול ערך מחושב" (`is-input` / `calcnum` / `is-override`) חלה כרגע על **גיליון
  התקציב ופוטר הצוות בלבד**. מסכי ההגדרות, הדיווח והחשבונות עדיין בשפה הישנה (שדה נראה כשדה רק ב-hover).
