...
- 📌 **סיווג צוות (בדיקת נאותות מול השאר) — מבוקש לעדכון הבא.** שדה קבוע על הצוות
  (למשל `team.kind: 'dd' | 'regular'`, ברירת מחדל `regular`) — **לא** התאמה לפי שם הצוות,
  כי "בדיקת נאותות" הוא שם צוות מקרי בנתוני הדמו. משם הפרדה בכל החיתוכים:
  · טאב תקציב — סיכום נפרד לצוותי DD מול השאר (מעל הסיכום המצרפי הקיים), וסימון במפת התקציב;
  · טאב בקרה — תקציב מול ביצוע ותחזית בשתי הקבוצות;
  · טאב תחקיר — תמהיל דרגות, מוקדי חריגה ופילוח לפי אדם, מופרדים;
  · **ייצוא**: עמודת "סוג צוות" + שורות סיכום ביניים בגיליון התקציב (`exportDealCSV`),
  ואותו פילוח בייצוא התחקיר ובמעקב.
  המנגנון קיים כבר חלקית — `model.aggregateTeams(snapshot, teamIds)` מחשב מצרף לקבוצת צוותים
  (משמש את סימון הצוותים הידני); הסיווג הוא הגרסה **הקבועה** שלו, ולכן המימוש הוא בעיקר
  שדה + שתי קריאות ל-`aggregateTeams` + הצגה, ולא מנוע חדש.
- **ייצוא התקציב קבור בהגדרות** ⚠️ — `export-deal` מפיק xlsx מלא (גיליון "תקציב" עם כל צוות/שורה
  וסיכומים + "תעריפים אפקטיביים" כשהתקרה חלה), אבל הכפתור יושב ב**הגדרות → פעולות על העסקה**,
  ליד "מחק עסקה". המשתמש שאל אם בכלל קיימת אפשרות ייצוא — כלומר הפיצ'ר קיים ולא נמצא.
  לתקן: כפתור ייצוא גם בראש טאב התקציב (להשאיר גם בהגדרות).
